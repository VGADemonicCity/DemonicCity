...
namespace DemonicCity.BattleScene
{
    ...
    public class EnemyAttackState : StatesBehaviour
    {
        ...
        void Start()
        {
            ...
            {
                if (state != BattleManager.StateMachine.State.EnemyAttack || m_battleManager.m_StateMachine.PreviousStateIsPause || m_battleManager.m_StateMachine.PreviousStateIsDebugging) // StateがEnemyAttack以外の時は処理終了
                {
                    return;
                }

                // もし敵パネルを選択して遷移してきた時
                if (m_battleManager.m_StateMachine.PreviousStateWithoutPauseAndDebugging == BattleManager.StateMachine.State.PlayerChoice)
                {
                    // 敵の攻撃力を上げて、エフェクトを再生
